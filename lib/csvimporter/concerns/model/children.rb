    ...
    module Children
      ...
      def append_child(source, options = {})
        ...
        self.class.has_many_relationships.each do |relation_name, child_class|
          ...
          return self.class.new(source, options) unless child_row_model.valid?
      ...
      def children_public_send(method_name)
        self.class.has_many_relationships.keys.flat_map do |relation_name|
      ...
      class_methods do
        ...
        # @param [Csvimporter::Import] row_model_class class of the relation
        def has_many(relation_name, row_model_class)
          raise "for now, Csvimporter's has_many may only be called once" if @_has_many_relationships.present?
